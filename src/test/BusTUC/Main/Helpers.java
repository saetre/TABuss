...
public class Helpers
{
	...
	public static ArrayList <String>readStuff(InputStream is)
	{
		...
		for (int i = 0; i < value.size(); i++)
		{
			...
			if (!value.get(i).isTransfer())
			{
				if (isTransfer)
				{
					...
					text.add((i + 1) + ": OVERGANGSFORSLAG " + (i)
	 ...
	 */
	public static ArrayList<Route> computeRealTime(Route[] foundRoutes,
			Route[] routes, HashMap<Integer, Integer> realTimeCodes,
			Browser k_browser, boolean afterTransfer, ArrayList <String> m_list)
	{
		...
		try
		{
			returnRoutes = Helpers.setTimeForRoutes(foundRoutes, realTimeCodes,
					k_browser, calculator, afterTransfer,m_list);
	...
	public static ArrayList<Route> handleMissedTransfer(ArrayList<Route> value)
	{
		...
		for (int i = 0; i < value.size(); i++)
		{
			...
			} else
			{
				...
				int walk = 2;
				System.out.println("Reisetid fra "
						+ value.get(i - 1).getBusStopName() + ": "
						+ value.get(i - 1).getArrivalTime() + " Reisetid: "
						+ value.get(i - 1).getTravelTime() + " og Avgang "
						+ value.get(i).getBusStopName() + " er: "
						+ value.get(i).getArrivalTime());
				System.out.println("Sammenligner verdier: "
						+ (Integer.parseInt(value.get(i - 1).getArrivalTime())
								+ Integer.parseInt(value.get(i - 1)
										.getTravelTime()) + walk) + " og "
						+ (Integer.parseInt(value.get(i).getArrivalTime())));
				// Assume we neeed minimum two minutes to get to the next bus
				// stop
				if ((Integer.parseInt(firstDest.getArrivalTime())
						+ Integer.parseInt(firstDest.getTravelTime()) + walk) >= (Integer
							.parseInt(transfer.getArrivalTime())))
				{
					...
					if (value.get(i - 1).getArrivalTime().length() == 3)
					{
						beforeTwelve = beforeTwelve
								+ value.get(i - 1).getArrivalTime();
					} else
						beforeTwelve = value.get(i - 1).getArrivalTime();
					int arrivalTimeHours = Integer.parseInt(beforeTwelve
							.substring(0, 2));
					int travelTime = Integer.parseInt(value.get(i - 1)
							.getTravelTime());
					System.out.println("Travel time: " + travelTime);
					int arrivalTimeMinutes = Integer.parseInt(beforeTwelve
							.substring(2, 4))
							+ (arrivalTimeHours * 60)
							+ travelTime;
					System.out
							.println("Arrival minutes: " + arrivalTimeMinutes);
					...
					if (buf.length() == 1)
						buf.insert(0, "0");
					String newTime = String.valueOf(newHours)
							+ String.valueOf(buf.toString());
					...
					String query = "fra " + departureStop + "," + "til "
							+ destination + " etter " + arrivalTime;

					BusStop stop = new BusStop(null, 0, value.get(i)
							.getBusStopNumber(), value.get(i).getBusStopName());
					...
					try
					{
						...
						ArrayList<Route> routes = Helpers.runString(
								destination, newList, Homescreen.k_browser,
								Homescreen.realTimeCodes, query);
						finalRoutes = new ArrayList<Route>();
						// Assure that no routes leave before we arrive at the
						// stop
						...
						for (int j = 0; j < finalRoutes.size(); j++)
						{
							System.out.println("FINAL ROUTES: "
									+ finalRoutes.get(j).getBusNumber());
						}
						// Add final routes to original list
						value.addAll(finalRoutes);
						for (int j = 0; j < value.size(); j++)
						{
							System.out.println("RETURN ROUTES: "
									+ value.get(j).getBusNumber());
						}
						return value;
					...
					} catch (Exception e)
					{
						...
						ArrayList<String> err = new ArrayList<String>();
						err.add(e.toString());
						SDCard.generateNoteOnSD("errorParseDataTransf", err,
								"errors");
						return null;
					}
	 ...
	 */
	public static ArrayList<Route> run(String input,
			ArrayList<BusStop> tSetExclude, Browser k_browser,
			HashMap<Integer, Integer> realTimeCodes, ArrayList <String> m_list)
	{
		Route[] finalRoutes;
		...
		if (!tSetExclude.isEmpty())
		{
			try
			{
				...
				else
				{
					...
					jsonSubString = jsonSubString.substring(0,
							jsonSubString.length() - 1);
					...
					// Compute real time
					returnRoutes = computeRealTime(finalRoutes, routes,
							realTimeCodes, k_browser, false, m_list);
	 ...
	 */
	public static ArrayList<Route> runString(String input,
			ArrayList<BusStop> tSetExclude, Browser k_browser,
			HashMap<Integer, Integer> realTimeCodes, String additional)
	{
		Route[] finalRoutes;
		// Perform action on clicks
		if (!tSetExclude.isEmpty())
		{
			try
			{
				...
				else
				{
					...
					for (int i = 0; i < routes.length; i++)
					{
						System.out.println("CALCULATED ROUTES: "
								+ routes[i].getBusStopName());
					}
					// Set walking dist
					Helpers.setWalkingDistance(routes, tSetExclude);
					calculator.printOutRoutes("BEFORE", routes, false);
					...
					{
						System.out.println("CALCULATED FINALROUTES "
								+ finalRoutes[i].getBusStopName());
					}
					calculator.printOutRoutes("AFTER", finalRoutes, false);
					ArrayList <String> m_list = new ArrayList <String>();
					// Compute real time
					ArrayList<Route> returnRoutes = computeRealTime(
							finalRoutes, routes, realTimeCodes, k_browser, true,m_list);
					/*
					 * ArrayList <Route> returnRoutes = new ArrayList <Route>();
					 * for (int i = 0; i < finalRoutes.length; i++) {
	...
	/*
	 * Method which parses the return message from BussTUC Not used with retros
	 * server
	 */
	public static boolean parseHtml(String[] html_page, StringBuilder str)
	{
		for (int i = 0; i < html_page.length; i++)
		{
			...
			// Simple error handling. If the object contains "error", return
			// false
	...
	public static Route[] setTimeForRoutes(Route[] finalRoutes,
			HashMap<Integer, Integer> realTimeCodes, Browser k_browser,
			final Calculate calculator, boolean afterTransfer, ArrayList <String> m_list)

	{
		...
		try
		{
			for (int i = 0; i < tempRoutes.length; i++)
			{
				int tempId = Integer.parseInt(realTimeCodes.get(
						tempRoutes[i].getBusStopNumber()).toString());
				...
				// Create new threads for sending queries to the real-time
				// system.
				...
				Thread thread = new Thread(new Runnable()
				{
					public void run()
					{
						...
						final BusDeparture tempNextBus = tempBrowser
								.specificRequest(tId, wLine, fList);

						// If not part of a transfer query, i.e new query based
						// on not reaching or bus
						if (!a_transfer)
						{
							tempRoutes[j].setArrivalTime(tempNextBus
									.getArrivalTime().getHours()
									+ ""
									+ String.format("%02d", tempNextBus
											.getArrivalTime().getMinutes())
									+ "");
							int k_totalTime = calculator.calculateTotalTime(
									tempRoutes[j].getArrivalTime(),
									tempRoutes[j].getTravelTime());
		...
		System.out.println("TIME RETRIEVING REAL-TIME: " + second
				/ 1000000000.0);
		// Remove suggestions where buses passing same stops have huge total
		// time difference
